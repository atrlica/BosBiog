...
ref <- as.data.table(read.csv("docs/REF_SPECIES.csv"))
...
# AGbiom.fun(biom.key[1,2], biom.key[1,3], 18.7)
# AGbiom.fun(biom.key[2,2], biom.key[2,3], 42.6)
...
## now re-run the previous AG analysis, and look to see if there is a stable ratio or biomass~density function to strike
...
andy.dbh <- read.csv("H:/FragEVI/processed/andy.dbh.growth.iter.csv")
andy.dbh <- andy.dbh[,-1]
andy.dbh <- as.data.table(andy.dbh)
